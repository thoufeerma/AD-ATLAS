...
console.log("\n[Content]");
{
  ...
  ok(Number.isInteger(s.json.data.returns?.windowDays) && typeof s.json.data.returns.accepted === "boolean", "returns policy in public settings", `${s.json.data.returns?.windowDays} days`);
  const returnsPage = await call("GET", "/pages/returns");
  ok(JSON.stringify(returnsPage.json.data.body).includes("{{return_window_days}}"), "returns policy prints the window from settings");
