...
| Tax Settings, product HSN code and GST rate | The GST tax invoice each order gets when it ships: a printable page linked from Track Order and the account's order history. Checkout and saved addresses pick the state from a list, since it decides CGST + SGST or IGST |
...
`{{support_email}}`, `{{return_window_days}}`, `{{legal_entity}}` and others listed in the page editor — so it
